package mod.bitsnblocks.chiseling.modes.sphere;
...
import mod.bitsnblocks.api.multistate.accessor.world.IInWorldStateEntryInfo;
import mod.bitsnblocks.api.multistate.accessor.world.IWorldAreaAccessor;
...
public class SphereChiselMode extends AbstractCustomRegistryEntry implements IChiselMode {
    ...
    private static VoxelShape buildSphereShape(final StateEntrySize size, final int diameter) {
        final int blocks = StateEntrySize.ONE_HALF.getBitsPerBlockSide() / size.getBitsPerBlockSide();
        final int fullBitAccuracy = blocks * StateEntrySize.ONE_HALF.getBitsPerBlockSide();
    ...
    public VoxelShape getShape(final IChiselingContext context) {
        ...
        final int blocks = StateEntrySize.ONE_HALF.getBitsPerBlockSide() / StateEntrySize.current().getBitsPerBlockSide();
        final int fullBitAccuracy = blocks * StateEntrySize.ONE_HALF.getBitsPerBlockSide();
        final int centerBitCoord = fullBitAccuracy / 2;
